...
define('TW_USE_EXTENSION', mb_strlen($routing['extension']) > 0);

/* prepare uri, query, script */
$routing['request_uri'] = $_SERVER['REQUEST_URI'];

/* remove query from string */
if (mb_strpos($routing['request_uri'], '?') > 0) {
    $routing['request_uri'] = mb_substr($routing['request_uri'], 0, mb_strpos($routing['request_uri'], '?'));
}

/* if page is run from a subfolder of a domain
 * the exact path will be stored in $routing['folder'] */
$routing['folder'] = mb_substr($_SERVER['SCRIPT_NAME'], 0, -9);
...
$routing['request'] = mb_substr($routing['request_uri'], mb_strlen($routing['folder']));
if (mb_strlen($routing['request']) > 0) {
    $routing['request_array'] = explode('/', $routing['request']);
} else {
    $routing['request_array'] = [];
}
