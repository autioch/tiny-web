...
class qbWebForm
{
    ...
    public function __construct($id = 'form', $action = '', $method = 'get', $class = '')
    {
        $this->id = $id;
        $this->action = $action;
        $this->method = $method;
    ...
    public function db_save()
    {
        ...
        $ignore = ['antybot', 'file', 'label', 'submit'];
        $fields = [];
        $values = [];
    ...
    {
        return mb_strlen($field['value']) > 2;
    }

    public function validate_textarea($field)
    {
        return mb_strlen($field['value']) > 10;
    }
