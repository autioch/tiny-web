...
function get_link($target, $description = '', $title = '', $class = '', $id = '') {
    ...
    /* define description */
    $des = (strlen($description) > 0) ? $description : $target;

    /* check if link is on the path to active page */
    $cla = $class;
    if ((strlen($target) > 0) && (strpos(TW_SELECTED, $target) === 0)) {
        if (strlen($cla) > 0) {
            $cla .= ' active';
        } else {
            $cla = 'active';
...
function get_download($file, $description = '', $title = '', $class = '', $id = '') {
    ...
    $des = (strlen($description) > 0) ? $description : $file;
...
function get_email($email, $description = '', $title = '', $class = '', $id = '') {
    ...
    $des = (strlen($description) > 0) ? $description : $email;
 ...
 * @param integer $width width attribute of image
 * @param integer $height height attribute of image
 * @return string full <img> tag.
 */
function get_image($src, $title = '', $class = '', $id = '', $width = '', $height = '') {
    $tar = ' src="' . TW_FOLDER . 'images/' . $src . '"';
    $wid = (strlen($width) > 0) ? ' width="' . $width . '"' : '';
    $hei = (strlen($height) > 0) ? ' height="' . $height . '"' : '';
    $al_ = ' alt="' . ( (strlen($title) > 0) ? $title : $src ) . '"';
 ...
 * @param integer $width width of the object in the page
 * @param integer $height height of the object in the page
...
function get_link_language($language, $description = '', $title = '', $class = '', $id = '') {
    ...
    $des = (strlen($description) > 0) ? $description : $language;

    $cla = (TW_LANGUAGE == $language) ? 'current' : '';
    $cla.= ( (strlen($cla) > 0) && (strlen($class) > 0) ) ? ' ' : '';
    $cla.= $class;
