...
class qbWebForm {
        ...
        <div class="form-container" id="<?php echo $this->id ?>-container">
            <?php foreach ($this->fields as $id => $field): ?>
                <div class="form-field form-field-<?php echo $field['type'], $field['valid'] ? '' : ' invalid' ?>">
                    <?php if ($field['type'] == 'label'): ?>
                        <div class="form-field-title"><?php echo $this->render_label($field) . "\n"; ?></div>
                    <?php else: ?>
                        <div class="form-field-title"><?php echo $this->html_label($field) . "\n"; ?></div>
    ...
    public function render_checkbox($field) {
        ...
        return '<input class="form-checkbox"' . $checked . ' name="' . $this->html_name($f) . '" id="' . $this->html_id($f) . '" type="checkbox"> ';
    }
    ...
    public function render_file($field) {
        $f = $this->render_get_field($field);
        return '<input class="form-file" name="' . $this->html_name($f) . '" id="' . $this->html_id($f) . '" type="file"> ';
    }

    public function render_hidden($field) {
        $f = $this->render_get_field($field);
        return '<input class="form-hidden" name="' . $f['name'] . '" id="' . $this->html_id($f) . '" value="' . $f['value'] . '" type="hidden"> ';
    }

    public function render_input($field, $type) {
        $f = $this->render_get_field($field);
        return '<input name="' . $this->html_name($f) . '" class="form-text form-' . $f['type'] . '" id="' . $this->html_id($f) . '" value="' . $f['value'] . '" type="' . $type . '"> ';
    }
    ...
    public function render_radio($field) {
        $f = $this->render_get_field($field);
        $result = '<ul class="form-radio">';
        foreach ($f['options'] as $key => $val) {
            $id = $this->html_id($f) . '-' . $key;
            $result .= '<li class="form-radio-item"><input' . (strcmp($f['value'], $key) == 0 ? ' checked="checked"' : '') . ' id="' . $id . '" name="' . $this->html_name($f) . '" value="' . $key . '" type="radio">';
    ...
    public function render_select($field) {
        $f = $this->render_get_field($field);
        $result = '<select class="form-select" name = "' . $this->html_name($f) . '" id = "' . $this->html_id($f) . '">';
    ...
    public function render_submit($field) {
        $f = $this->render_get_field($field);
        return '<input class="form-submit" name="' . $this->html_name($f) . '" id="' . $this->html_id($f) . '" value="' . $f['value'] . '" type="submit"> ';
    }
    ...
    public function render_textarea($field) {
        $f = $this->render_get_field($field);
        return '<textarea class="form-textarea" name="' . $this->html_name($f) . '" id="' . $this->html_id($f) . '">' . $f['value'] . '</textarea>';
    }
